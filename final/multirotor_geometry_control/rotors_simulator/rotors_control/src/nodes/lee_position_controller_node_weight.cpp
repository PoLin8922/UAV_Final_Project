	 ...
	 private_nh_(private_nh)
{
	...
	weight_state_sub_ = nh_.subscribe("/delivery_state", 1, &LeePositionControllerNode::StateCallback, this);
...
void LeePositionControllerNode::InitializeParams()
{
	...
	std::cout << "mass: " << lee_position_controller_.vehicle_parameters_.mass_ << " kg" << std::endl;
	if(weight_state == 1) {
		lee_position_controller_.vehicle_parameters_.mass_ += 1.51/4;
		std::cout << "Total mass with box: " << lee_position_controller_.vehicle_parameters_.mass_ << " kg" << std::endl;
	}
...
void LeePositionControllerNode::StateCallback(const std_msgs::Int16::ConstPtr& state_msg)
{
	weight_state = state_msg->data;
	InitializeParams();
	std::cout << "Reinitialize mass!!" << std::endl;
}
