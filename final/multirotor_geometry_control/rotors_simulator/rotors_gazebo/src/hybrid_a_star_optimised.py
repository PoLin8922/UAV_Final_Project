...
def main():
    ...
    x_pub = rospy.Publisher('waypoints_x', Float64, queue_size=10)
    y_pub = rospy.Publisher('waypoints_y', Float64, queue_size=10)
    ...
    sx, sy, stheta= 11, 8, 0
    gx, gy, gtheta = 13.7, 19, 0 #2,4,0 almost exact
    ...
    for i in range(8,22):
        expand.append((i,11))
        obstacle.append((i,11))
        expand.append((i,17))
        obstacle.append((i,17))
    for j in range(12,17):
            expand.append((8,j))
            obstacle.append((8,j))
            expand.append((21,j))
            obstacle.append((21,j))

    #house3
    for i in range(-2,11):
        expand.append((i,25))
        obstacle.append((i,25))
        expand.append((i,34))
        obstacle.append((i,34))
    for j in range(26,34):
            expand.append((-2,j))
            obstacle.append((-2,j))
            expand.append((10,j))
            obstacle.append((10,j))
    ...
    hy_a_star = hybrid_a_star(-15, 35, -15, 40, obstacle=obstacle, resolution=1, vehicle_length=1.1)
    path = hy_a_star.find_path((sx,sy,stheta), (gx,gy,gtheta))
    print(len(path))
    rx, ry = [], []
    for node in path:
        x_pub.publish(node[0])
        rx.append(node[0])
        y_pub.publish(node[1])
        ry.append(node[1])
        rospy.sleep(0.1)
